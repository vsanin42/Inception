...
if [ ! -d /var/lib/mysql/mysql ]; then
	...
	mariadbd --user=mysql --skip-networking --socket=/run/mysqld/mysqld.sock --datadir=/var/lib/mysql &
	tmp_pid=$!

	while ! mariadb-admin ping --socket=/run/mysqld/mysqld.sock >/dev/null 2>&1 ; do
